#!/usr/bin/env bats
# SOURCE: fleet/deploy/tests/forge_runner_apply.bats
# AUTHOR: alice
...
stub_delegue() { # stub_delegue <rc> [ligne a ecrire sur stdout]
  local d="$BATS_TEST_TMPDIR/repo/fleet/deploy/docker"
  mkdir -p "$d"
...
EOF
  ...
  # `repo_root()` remonte trois crans depuis `<racine>/fleet/deploy/lib`.
  ...
  # l'utilisait le plus.
  mkdir -p "$BATS_TEST_TMPDIR/repo/fleet/deploy/lib"
  local cible="$BATS_TEST_TMPDIR/repo/fleet/deploy/lib/provision-lib.sh"
