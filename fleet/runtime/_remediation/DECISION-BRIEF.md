...
- **F-C026 (flaggé, doc-drift confirmé)** : `sp_drafts/protocole-user-worker.md` se dit « Statut : draft POC » (protocole actif) + pointe 2× `agent-worker-base.md` (**fichier inexistant**). Fix = statut réel + successeur du fichier mort → besoin contexte produit (quel est le SP courant qui remplace agent-worker-base ?). Non-devinable seul.
- **F-C023 (D6-SSoT)** : le system-prompt architect actif serait un draft historique hors block-SoT (`sp_builder/blocks.ex`). SSoT sp_builder → décision produit (câbler l'architect SP dans les blocks vs garder le draft).
