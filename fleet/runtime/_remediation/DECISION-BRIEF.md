...
## Sweep « lignes DOCTRINE non-taguées » (watchdog-catch — bord prématuré rattrapé)
Le count LEDGER a révélé 29 lignes DOCTRINE `unassigned` (verdicts cluster jamais reportés par-ligne). Toutes taguées.
Le sweep a attrapé des « mal-rangés » que j'avais classés mais jamais exécutés/vérifiés au code :
- **F-C040 FIXÉ** ✅ : `apps/fleet_spawner/README.md` affirmait que TOUS les `:fleet_spawner, :*` sont env-drivés via `LCARS_*` + « full catalogue in template » — FAUX. runtime.exs n'en rend env-drivés qu'un sous-ensemble (`:boot_permanent_at_start`/`:state_fs_root`/`:tmux_sock_base`/`:mcp_server_spec`/`:bwrap_launch_path`/`:cap_profiles_dir`) ; le reste est app-env (`:pod_dir_root` a même perdu `LCARS_PODS_ROOT` en 2026-06-09). README réécrit fidèle.
- **F-C049 — CORRECTION R-14 de mon propre brief** : j'avais écrit « dep `fleet_event_router` vestigiale » — FAUX au code. La dep **compile-time** (`deps()`) est NÉCESSAIRE (`%Fleet.Event{}` matché dans result_event_test.exs). Seule l'entrée `extra_applications` est vestigiale (Bridge retiré Z7.3) MAIS délibérément gardée pour l'ordre de boot (risque #576 « unknown registry Fleet.PubSub »), flaggée par l'auteur « candidate au retrait AVEC PREUVE ». Retrait non-prouvable par `mix test` (pas de boot-release live) → **hygiène-deferred, décision** (pas un fix trivial). Leçon : j'ai classé depuis le rapport, pas le code (R-14 sur MON brief).
- **F-C061 — edge-case fail-open VÉRIFIÉE (correctement DOCTRINE)** : `RoleDispatch.load_role_or_skip` rend `{:skipped, :no_role}` **silencieux** (pas de Logger) sur un login au rôle non-chargeable ; `dispatch_by_verdicts` ne dispatche que `hd(pending)`. Si un **humain** requesté reviewer est en tête de `pending` avant un juge fleet → le juge fleet est **affamé** (skip en boucle, la PR ne progresse pas). Ne se matérialise que selon l'ordre `union(requested_reviewers, jury)` + backstops poller. **Décision design** : filtrer les rôles non-résolvables avant `hd` (ou itérer pending). Touche un chemin review live → ton call. (tag `D-review-fork`.)
- **F-C026 FIXÉ** ✅ (le « besoin contexte produit » était un PUNT — investigable au grep) : `protocole-user-worker.md` est ACTIF (`Pod.Assets.read_protocole_user/0` l'injecte à chaque spawn, assets.ex:81) → label « draft POC » corrigé en « actif » ; `agent-worker-base.md` (référencé 2×) n'existe pas → les SP-base sont désormais par rôle (`agent-<role>-base.md`), pointeur corrigé.
- **F-C023 (D6-SSoT — prémisse VÉRIFIÉE, documenté-intentionnel)** : l'architect SP est bien un draft historique hors block-SoT, MAIS `sp-map.yaml:8` le documente explicitement — « `architect` (user-facing, outside the pod socle) is NOT here — it keeps its historical draft (a different socle: a human in front) ». Aucun bloc `role/architect*`. → **décision documentée-délibérée** (façon D5-keep) : garder le socle architect séparé (human-facing) vs le migrer dans les blocs (au prix de perdre la spécificité human-facing). Ma reco : garder (documenté + raison valable), sauf si tu veux les invariants core/method partagés aussi sur l'architect.
