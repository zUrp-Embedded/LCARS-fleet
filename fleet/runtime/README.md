...
refuses divergence. Omitted for readability: the foundation utilities (`deps: []`: `Fleet.Decision`, `Fleet.EnvParse`, `Fleet.Event`, `Fleet.GitRef`, `Fleet.Labels`, `Fleet.Layout`, `Fleet.SchemaCache`, `Fleet.Shutdown.Quiesce`, `Fleet.Slug`), which every domain may depend down on, and the OTP root `Fleet.Application`,
