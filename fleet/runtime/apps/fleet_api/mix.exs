defmodule Fleet.API.MixProject do
  ...
  defp deps do
    [
      ...
      # F-010 : readiness sonde la liveness du rail step (Ring 3 fleet_pilot) via
      # `Fleet.Pilot.Application.step_status/0` — dép vers le bas (Ring 4 → Ring 3), acyclique.
      ...
      # bas (Ring 4 → Ring 2), acyclique (fleet_mcp ne dépend pas de fleet_api).
      ...
      # dép vers le bas (Ring 4 → Ring 2), acyclique (fleet_starfleet ne dépend pas de fleet_api).
      {:fleet_starfleet, in_umbrella: true},
      # MA-18 : valide le cap-profile AVANT d'ACK un `/api/admin/spawn` (même loader que le
      # PublishConsumer, source unique). Dép explicite (était transitive via fleet_pilot) — Ring 4 → Ring 0.
