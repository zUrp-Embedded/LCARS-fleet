defmodule Fleet.ShutdownTest do
  @moduledoc """
  DN ring0/lcars-fleet_service §Fleet.Shutdown. `async: false` :
  ...
  defmodule StubDispatcher do
    ...
    def refuse_new_jobs(_opts) do
      Agent.update(Fleet.ShutdownTest.Box, fn s -> %{s | refused: true} end)
      :ok
    end

    @impl true
    def in_flight_count do
      Agent.get_and_update(Fleet.ShutdownTest.Box, fn
        %{seq: [h | t]} = s -> {h, %{s | seq: t}}
        %{seq: []} = s -> {0, s}
      end)
    end
  ...
  defp start_sd(opts) do
    ...
    {:ok, _} = start_supervised({Fleet.Shutdown, [name: name] ++ opts})
    name
  end

  test "NoOp default → begin/drain :ok, drain immédiat (0 in-flight)" do
    name = start_sd([])
    assert :ok = Fleet.Shutdown.begin(name: name, grace_ms: 200)
    assert :ok = Fleet.Shutdown.drain_in_flight(name: name, grace_ms: 200)
  end

  test "backend séquence [2,1,0] → drain réel converge" do
    box([2, 1, 0])
    name = start_sd(dispatcher: StubDispatcher)
    assert :ok = Fleet.Shutdown.drain_in_flight(name: name, grace_ms: 5_000)
  ...
  test "backend toujours >0 → drain timeout mais :reply :ok (shutdown continue)" do
    ...
    name = start_sd(dispatcher: StubDispatcher)
    assert :ok = Fleet.Shutdown.drain_in_flight(name: name, grace_ms: 300)
  end

  test "begin appelle refuse_new_jobs" do
    box([0])
    name = start_sd(dispatcher: StubDispatcher)
    assert :ok = Fleet.Shutdown.begin(name: name, grace_ms: 300)
