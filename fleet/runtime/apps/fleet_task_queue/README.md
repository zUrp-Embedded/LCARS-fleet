...
- `list_active/0` — work items ACTIFS (`:pending`/`:assigned`/`:in_progress`, autorité `@active_states`). Consommé par la réconciliation de verrous du poller (G1) : une brique sous éval gatekeeper active est possédée ; une éval `:cleared` (supersédée) ou `:completed` ne l'est plus. Query.
