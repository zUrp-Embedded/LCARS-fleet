defmodule Fleet.Pipeline.Gates.Predicate do
  ...
  gate `human_approval_required`) est dans `Fleet.Pipeline.Gates`.
  """
  ...
  ## Examples
      ...
      iex> alias Fleet.Pipeline.Gates.Predicate
      iex> Predicate.eval?("spec_doc_exists AND spec_doc_non_empty",
      ...>   %{"spec_doc_exists" => true, "spec_doc_non_empty" => true})
      true

      iex> Fleet.Pipeline.Gates.Predicate.eval?("severity_max != critical",
      ...>   %{"severity_max" => "important"})
      true

      iex> Fleet.Pipeline.Gates.Predicate.eval?("tasks_count >= 1", %{"tasks_count" => 0})
      false
