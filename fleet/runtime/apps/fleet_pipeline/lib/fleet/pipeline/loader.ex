defmodule Fleet.Pipeline.Loader do
  ...
  ## Configuration
    ...
    * `:fleet_pipeline, :workflow_maps_root` — racine catalogue YAML
      (default `Application.app_dir(:fleet_pipeline, "priv/canon/workflow_maps")`)
  ...
  def load!(workflow_map_name, opts \\ []) when is_binary(workflow_map_name) and is_list(opts) do
    ...
    case ExJsonSchema.Validator.validate(schema, yaml) do
      ...
      {:error, errors} ->
        raise "Fleet.Pipeline.Loader: schema #{@schema_file} invalide pour #{workflow_map_name}: #{inspect(errors)}"
    end
  end
  ...
  # fleet_pipeline (autonome) : le Loader ne peut pas dépendre de WorkflowMapNav (fleet_pilot),
  ...
  defp validate_graph!(%{"steps" => steps}, workflow_map_name) do
    case Fleet.Pipeline.GraphValidator.validate(steps) do
      ...
      {:error, reason} ->
        raise "Fleet.Pipeline.Loader: workflow_map #{workflow_map_name} — #{Fleet.Pipeline.GraphValidator.describe(reason)}"
    end
  end
  ...
  defp workflow_maps_root(opts) do
    Keyword.get(opts, :workflow_maps_root) ||
      Application.get_env(:fleet_pipeline, :workflow_maps_root) ||
      Application.app_dir(:fleet_pipeline, "priv/canon/workflow_maps")
  end
