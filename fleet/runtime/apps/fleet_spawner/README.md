...
**Date** : 2026-05-09
**Dernière révision** : 2026-07-05 (dédup B4 : glob des jsonl de session → autorité unique `Pod.SessionFiles` (callers gc/liveness/seed-store) ; retrait du sock-dir post-kill → `PodTmux.remove_sock_dir/1` (teardown + PodWarden) ; grace 2-tick du `PodWarden` → cœur pur partagé `grace_2tick/2` ; résolveurs launcher de `Pod.Backend` → une fn privée `launcher_path/2` ; cap-profile effectif → `Fleet.CapProfile.with_project/2`. 2026-07-02 : resync gen_statem — `Fleet.Spawner.Pod` est un `gen_statem` atomisé en sous-modules `pod/*`, ex-wrappers/`defdelegate` retirés, appels directs sur les sous-modules.)
...
sont les phases du cycle, atomisé en 14 sous-modules `pod/*` (chacun une île sans
...
- **`data.conditions`** (MapSet) accumule les jalons franchis (`:home_projected`, `:context_injected`,
  ...
  (un pipe git_native entre son submit et la confirmation `deliverable.published`).
...
- `Fleet.Spawner.wake_pod/1` — réveil d'un pod long-lived : touche le `turn.flag` (rail porteur) puis `cast` `:rearm_deadline` + `:arm_kick` (le FALLBACK send-keys `wake` ne part que si le pull n'arrive pas). N'affirme PAS que l'agent s'est réveillé (succès réel = l'ACK observé par la boucle)
...
- `Fleet.Spawner.Pod` — le `gen_statem` du lifecycle (`callback_mode [:handle_event_function, :state_enter]`) : 8 états = les 8 phases (cf. Machine à états). Atomisé en 14 sous-modules `pod/*` (chacun une île sans state/Port/timer, documentés ci-dessous). Public : `start_link/1`, `name/1` (nom registry-via)
...
- `Fleet.Spawner.Pod.Scaffold` — **PRÉPARATION du substrat disque du pod** (assets, brief, workspace, recall), extraite de `Pod` (aucun Port/timer/state machine — chaque étape rend `:ok`/`{:ok, _}` ou un `{:error, reason}` taggé que le `with` de l'état `:projecting` propage vers `transition_failed`). Le module N'ORCHESTRE PAS : les ÉTATS `:cleaning`/`:projecting` RESTENT au cœur du `Pod` (leur `with` est l'orchestrateur), Scaffold n'expose que les ÉTAPES. Le `Pod` lui passe le `state` (ou le `cap_profile`) en argument ; aucune dépendance vers `Pod` (pas de cycle). Dépend de `Pod.Fs` (écritures FS non-bang), `Pod.LaunchSpec` (cwd/projet effectif), `Pod.TaskProbe` (gate d'enqueue), `Pod.SessionFiles` (glob partagé des jsonl), `Fleet.SPBuilder` (filtre skills) ; et en pleine qualif `Fleet.CapProfile` (source unique du `name` + `with_project/2`), `Fleet.ProjectBootstrap.Phase.Clone` (clone workspace + doc), `Fleet.Spawner.SeedStore` (restore recall), `Fleet.TaskQueue` (enqueue), `Fleet.Slug`, `Application` (config + assets `priv/`) :
  ...
  - `issue_id_to_filename(issue_id)` — `/`→`_` (filename safe, garde `#`) ; appelé pour le `issues/<id>.md` ET le brief.
  - `default_brief(state)` — corps du `issues/<id>.md` (rôle résolu via `Fleet.CapProfile.name`, brief `opts[:brief]` ou placeholder).
...
- `Fleet.Spawner.Pod.Backend` — **VIE & MORT du backend OS du pod** (teardown Port/holder, socket MCP, résolveurs launcher), extraite de `Pod`. Tout ce qui TUE le process OS du pod et libère ses ressources host ; le module N'ORCHESTRE PAS : les CALLBACKS/ÉTATS (`terminate/3`, `handle_event({:call, from}, :kill, …)`, les états `:releasing`/`:launching`/`:projecting`, la fn `do_launch_backend`) RESTENT au cœur du `Pod`, ils appellent `Backend.*` pour le geste OS. Le `Pod` lui passe le `state` (ou un `port`/`pod_id`) en argument ; aucune dépendance vers `Pod` (pas de cycle). Alias `Fleet.Spawner.PodTmux` (`kill_holder`/`sock_path`/`alive?`) ; en plein qualif `Fleet.Spawner.LaunchBackend`, `Application`, et le SEAM RUNTIME `apply(mcp_socket_provisioner(), ...)` (provisionneur MCP résolu au runtime — `fleet_spawner` Ring 1 ne dépend PAS compile-time de `fleet_mcp` Ring 3 ; défaut atom littéral `Fleet.MCP.PodSocketSupervisor`, override test `:mcp_socket_provisioner`) :
  ...
  - `ensure_pod_socket(pod_id)` — crée la socket MCP per-pod AVANT le launch → `{:ok, socket_path}` (chemin host, le fichier DOIT exister avant le bind bwrap) ; appelé par l'état `:projecting`.
...
- `:fleet_spawner, :auth_mode` — **`:bind` UNIQUEMENT** (le mode `:token_arg` a été **retiré 2026-06-14**, plus de toggle) : bwrap bind RW le `.credentials.json` humain → refresh OAuth natif (proactif 5min + réactif 401 + lockfile), full scope, pas de falaise ~8h. Posé en `LCARS_AUTH_MODE=bind` (seule valeur acceptée par `bin/bwrap_launch.sh`). L'ex-`:token_arg` fuyait le token en argv ET ne refreshait pas (un eng >8h perdait l'auth en vol) → supprimé. La validation scope/plan du creds natif est portée par `Fleet.Credentials.Gate.validate/2` (entrée unique, app `fleet_credentials`) appelée depuis l'état `:launching` après l'auth-token : lecture **source unique** (un seul `File.read` + parse, gate scope/plan partagent CE parse). Le gating vivait jusque-là dans ce module (`Pod`) ; déplacé dans `fleet_credentials` pour réconcilier code et contrat — Pod ne garde que la résolution du chemin claudeDir (`claude_dir_for/1`).
...
- `:fleet_spawner, :mcp_socket_provisioner` — module du provisionneur de socket MCP per-pod (R9). **Seam runtime** vers `fleet_mcp` (Ring 3) : `fleet_spawner` (Ring 1) NE PEUT PAS en dépendre en `mix.exs` (dep inversée) → résolu au runtime (`Application.get_env` + `apply`, défaut atom littéral `Fleet.MCP.PodSocketSupervisor`, zéro dep compile-time, même pattern que `PodTools`→`ForgeClient`). `ensure_pod_socket/1` (état `:projecting`, avant launch) → `{:ok, socket_path}` ; `release_pod_socket/1` (filet `terminate/3`). Override test `Fleet.Spawner.MCPSocketStub` (rend un chemin SANS créer de vrai socket — mirror de `launch_backend: StubBackend`)
...
- `:fleet_spawner, :publish_deadline_ms` — fail-safe du generic timeout `:publish_deadline` (default **120_000**) : levée forcée du flag `:publishing` d'un pipe `git_native` si `deliverable.published` n'arrive pas (WARNING loggé)
