defmodule Fleet.Spawner.Pod.Assets do
  ...
  @doc """
  Role-aware SP draft: a role's draft is `agent-<role>-base.md` if it EXISTS (in `fleet_sp_builder`'s
  `priv/sp_drafts/`), otherwise the generic worker draft (yop workflow →
  get_work_item → submit_result). Catalogue convention (the draft follows `metadata.name`), no more
  role hard-coded in a `case`: the architect lands on its delegator draft, any role with no dedicated
  draft on the worker draft. `role` is interpolated into a path → validated via the slug
  smart-constructor (single source of the path-safe charset; a malformed `role` simply falls back to
  the default draft).
  """
  ...
  the native `Monitor` tool (cf. SP `agent-worker-base.md`) → wake-by-flag (`turn.flag` touched by
  the fleet), zero CONTENT send-keys. The asset lives in `fleet_spawner`'s `priv/` (resolved via
  `app_dir`, like the SP draft). chmod best-effort: the agent runs `bash ~/watch.sh`, the exec bit
  is not required.
  """
