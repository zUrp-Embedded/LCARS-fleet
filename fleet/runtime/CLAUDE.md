...
**Date** : 2026-05-26
**Dernière révision** : 2026-07-20 (règle de langue : ce qui part avec la boîte est en anglais — cf. Code conventions ; migration single-app + boundary du 2026-07-12 toujours en vigueur)
...
- **Langue — la règle** : *ce qui part avec la boîte est en ANGLAIS ; ce que la boîte énonce une fois livrée suit la langue de son opérateur.* Le test est la **livraison**, pas l'emplacement dans l'arbre.
  ...
  - **État réel, à ne pas confondre avec la règle** : la boîte n'est PAS conforme — il reste de la prose française dans `lib/`, `test/` et `bin/`. L'inventaire se mesure, il ne se recopie pas : `grep -rlE '[éèêàçùôîûïœ]' lib test bin` (plancher : l'accent rate le français non accentué). **Aucun gate ne verrouille cette règle aujourd'hui** — le mur anti-FR reste à construire, après la passe de traduction, scopé à la boîte (le package SP n'a rien à y exempter, il en est hors).
