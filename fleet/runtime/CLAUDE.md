...
**Boot** : `Fleet.Application` (l'UNIQUE callback OTP) démarre les superviseurs de domaine dans un ordre qui EST l'invariant (cicatrice F8 inline : event_router premier, mcp avant starfleet, starfleet après spawner). `max_restarts: 0` au sommet = mort d'un domaine → mort du node (sémantique umbrella conservée, cf. D-17 du chantier migration). Le check `boot.order_f8` de `mix lcars.contracts.check` verrouille l'ordre.
...
- **Préfixe des messages de log** : `<DernierSegmentModule>: <message>`. Deux rails délibérés font exception : `AUDIT <event.type>` et `pod <id> …`.
