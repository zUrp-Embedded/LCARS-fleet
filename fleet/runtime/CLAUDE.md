...
`Fleet.EventRouter.Bus` (Phoenix.PubSub) is the single broadcast/subscribe substrate. Apps publish to `fleet.events` and consume via `subscribe/1`. In `:test` env it is swapped to `Fleet.IpcFilter.EventBackend.NotWiredYet` (see "Test hermeticity" below).
...
2. `config/<env>.exs` — `test.exs` overrides for hermetic tests (NotWiredYet bus, StubBackend launcher, consumers off, `start_listener: false`)
...
- `fleet_ipc_filter, event_backend: NotWiredYet` — no parasitic Bus broadcasts in async tests
