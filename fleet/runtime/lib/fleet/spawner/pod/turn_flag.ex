defmodule Fleet.Spawner.Pod.TurnFlag do
  ...
  def write(pod_dir) when is_binary(pod_dir) do
    ...
    case File.write(flag, token <> "\n") do
      ...
      {:error, reason} ->
        Logger.warning(
          "TurnFlag: write #{pod_dir}: flag write failed (#{inspect(reason)}) — carrier rail silent (best-effort)"
        )
    ...
    e ->
      Logger.warning(
        "TurnFlag: write #{pod_dir}: flag write exception (#{inspect(e)}) — carrier rail silent (best-effort)"
      )
