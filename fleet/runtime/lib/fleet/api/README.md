...
(the web dashboard is one consumer among others). **No-auth by design** — the security contract
...
- `Fleet.API.Rest` — `Plug.Router` HTTP: no-auth reads + maps spawn-admission verdicts to HTTP statuses; forwards `/dashboard`
...
- `Fleet.API.Dashboard` — `Plug.Router` for the native Elixir dashboard (`/dashboard`, EEx compiled at build)
