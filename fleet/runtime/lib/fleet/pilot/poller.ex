defmodule Fleet.Pilot.Poller do
  ...
  **Last revised**: 2026-07-18
  ...
  # Cadence for RE-KICKING the arch as long as at least one issue is waiting (`lcars-awaits-arch`). Throttled:
  ...
  # poll_count nor call maybe_rekick_arch — counting them would compress "~5 min" to ~10s under
  # forge activity generated by the arch's own wake (self-amplified spend).
  ...
  defp do_poll(state, mode) do
    ...
    case forge.list_org_repos(state.org, state.forge_opts) do
      {:ok, repos} ->
        ...
        # Arch re-kick — FLEET-GLOBAL action on the UNIQUE arch pod, decided ONCE per tick on the
        # cross-repo union. Inside the per-repo loop it would fire R times per throttle-tick
        # (poll_count is loop-invariant): R repos awaits-arch = R wakes of the SAME arch + R log
        # lines each claiming "throttle" — a trace that lies about the throttle and undercounts
        # the backlog. REGULAR ticks only: a kick-poll firing it would compress the ~5 min throttle
        # to the webhook rate — and the woken arch generates the very webhooks that would re-wake it.
  ...
  # does NOT inject the seam → the REAL `Fleet.Spawner` is used — resolve the default HERE, never
  # a nil-guard on the seam (a guard that skips on nil turns this prod rail into a silent no-op).
  # A failed wake (arch pod dead, `{:error, :not_found}`) loses latency, never the backlog: the
  # label persists on the forge, this same re-kick fires again `@awaits_rekick_every` ticks later,
  # and the PermanentWarden respawns the arch in the interval. The log states the ACTUAL outcome —
  # logging "re-kick" before the wake would let the trace lie whenever the wake fails.
  defp maybe_rekick_arch(awaits_ids, %__MODULE__{} = state) do
    if awaits_rekick?(MapSet.size(awaits_ids), state.poll_count) do
      ...
      # The arch is a context-long/UNIQUE worker (project/forever) — the FORGE is
      ...
      # `get_work_item` RETURNS the ticket (otherwise the woken arch reads `{done:true}` and is
      # blind to the escalation backlog). Busy → we only wake
