...
> chaque humain). Ses trois outils, joués à l'install seulement, vivent dans `deploy/lib/` :
> `deploy-release.sh` (bâtit et pose la release), `enroll-catalogue.sh` (dérive le roster de la
> recette forge), `provision-role-tokens.sh` (minte les jetons de rôle). Leurs témoins sont dans
> `deploy/tests/lib/`, joués par `deploy/gate.sh`.
...
postent EN LEUR NOM via `<FORGE_ROLE_TOKENS_DIR>/<role>.gitea_token`. La pose est mécanisée :
    ...
    deploy/lib/provision-role-tokens.sh --forge <URL> --passwords-file <secrets.json> \
                                                                # les 6 rôles + le système = A4 complet
    deploy/lib/provision-role-tokens.sh --forge <URL> --check                          # sonde (nuke-drill)
...
Tests : `deploy/tests/lib/provision_role_tokens.bats` (bats, joué par `deploy/gate.sh`).
