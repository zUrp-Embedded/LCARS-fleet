...
- Knobs `:lcars_fleet, :spawner_*` — each is read by its OWNING module, and that `@moduledoc` is the authority for the knob's meaning and its default.
  **This card no longer lists them, and the reason is the list it used to carry.** It named ~20 keys under `:fleet_spawner` with no prefix; the namespace migration killed every one of those names and nothing went red — a map is the one artifact neither the gate nor a review filters. An inventory in a card expires by construction. The rule below does not.
