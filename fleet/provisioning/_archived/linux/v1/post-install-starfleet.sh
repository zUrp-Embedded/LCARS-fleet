...
# ─── gh auth — token depuis /home/private ────────────────────────────────────
GH_TOKEN_FILE="/home/private/gh-token"
sudo mkdir -p /home/private
sudo chmod 700 /home/private
if [ ! -f "$GH_TOKEN_FILE" ]; then
    if [ -t 0 ]; then
        warn "gh-token not found — interactive setup"
        ...
        echo
        echo "$GH_PAT" > "$GH_TOKEN_FILE"
        chmod 600 "$GH_TOKEN_FILE"
        info "gh-token saved to $GH_TOKEN_FILE"
    else
        warn "gh-token not found and non-interactive — gh auth skipped (create $GH_TOKEN_FILE manually)"
    fi
fi
if [ -f "$GH_TOKEN_FILE" ]; then
    ...
    warn "gh auth skipped — no token file"
fi
