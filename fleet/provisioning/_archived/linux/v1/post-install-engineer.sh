...
rm -f "$HOME/.lcars"
ln -s $LCARS_ROOT "$HOME/.lcars"
info "Symlink ~/.lcars → $LCARS_ROOT"

# Configure Anthropic plan reset schedule (skip if already set or non-interactive)
if [ -t 0 ]; then
    bash "$SCRIPT_DIR/configure-plan.sh"
else
    warn "configure-plan.sh skipped — non-interactive context (run manually: bash $SCRIPT_DIR/configure-plan.sh)"
fi
