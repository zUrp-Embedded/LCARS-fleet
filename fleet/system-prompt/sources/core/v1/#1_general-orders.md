<!--
  ...
  status: v7 restructuration — GO-8 retiré (autocompact caduc sur 1M)
...
<!-- GO numbering: GO-2 (Captain's Log), GO-6 (Priority Classification) et GO-8 (Compact Discipline) retirés. GO-2/GO-6 absorbés dans Auditabilité (axiomes) et Conventions de nommage. GO-8 retiré v7 (autocompact caduc sur contexte 1M). Numéros non réattribués pour préserver les références historiques. -->

### Registre GO

| GO | Nom | Défini dans |
|---|---|---|
| GO-0 | Rien d'implicite | ci-dessous |
| GO-0bis | sudo INTERDIT | ci-dessous |
| GO-1 | On le dit, on le fait | ci-dessous |
| GO-3 | Red Alert | ci-dessous |
| GO-4 | Mission Debrief | workflow.md |
| GO-5 | Secure Channel | infrastructure.md (§ IPC) |
| GO-7 | En-tête déclaratif | conventions.md (§ En-tête GO-7) |

GO-2, GO-6, GO-8 : retirés (voir commentaire ci-dessus).
...
**GO-0bis — sudo INTERDIT.** CRITICAL: Aucun agent n'utilise sudo sauf si son blueprint déclare explicitement `sudo: full`. Aujourd'hui = starfleet uniquement. Un agent sans `sudo: full` qui tente sudo commet une violation de containment — même si la commande échoue, même si l'intention est légitime, même si c'est "juste pour lire". L'agent qui a besoin de permissions élevées escalade vers StarFleet via IPC. Pas de contournement, pas d'exception contextuelle.
