...
flowchart TB
    ...
    Fleet_MCP -. "seam :forge_client / :project_onboard" .-> Fleet_Pilot
```

*(Generated by `mix lcars.topology` from the `use Boundary` declarations — the gate
refuses divergence. Omitted for readability: the foundation utilities (`deps: []`: `Fleet.Catalogue`, `Fleet.Conflict`, `Fleet.Decision`, `Fleet.DurableLog`, `Fleet.EnvParse`, `Fleet.Event`, `Fleet.GitRef`, `Fleet.Labels`, `Fleet.Layout`, `Fleet.Opts`, `Fleet.Publish.InFlight`, `Fleet.ReceptionFilter`, `Fleet.SchemaCache`, `Fleet.Shutdown.Quiesce`, `Fleet.Slug`), which every domain may depend down on, and the OTP root `Fleet.Application`,
