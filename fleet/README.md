...
refuses divergence. Omitted for readability: the foundation utilities (`deps: []`: `Fleet.Catalogue`, `Fleet.Conflict`, `Fleet.Decision`, `Fleet.DurableLog`, `Fleet.EnvParse`, `Fleet.Event`, `Fleet.FindingsWire`, `Fleet.GitRef`, `Fleet.Labels`, `Fleet.Layout`, `Fleet.Opts`, `Fleet.PodId`, `Fleet.Publish.InFlight`, `Fleet.ReceptionFilter`, `Fleet.ReleaseDoor`, `Fleet.SchemaCache`, `Fleet.Shutdown.Quiesce`, `Fleet.Slug`, `Fleet.SystemConfig`), which every domain may depend down on, and the OTP root `Fleet.Application`,
