#!/usr/bin/env bash
# SOURCE: fleet/test/probes/_archived/gate-r-core-comm-inc3b3.sh
...
# par test/fleet/mcp/pod_socket_test.exs (mix test — get_work_item/submit_result sur la socket AF_UNIX per-pod ;
...
echo "SUPERSEDED — gate e2e archi coffre/MCP-channel (pre-ADR-F/ADR-G). Reecriture = deploy-env. Preuve vivante : test/fleet/mcp/pod_socket_test.exs (mix test)"
exit 2
