...
| | why |
|---|---|
| **docker** | everything runs in containers — the LCARS container, the forge, the runner |
...
without leaving a half-installed container.
...
bash install.sh --container --bench                 # forge + container + runner, one gesture
```

`install.sh` is **the** door, and the only one. It looks at what your machine allows, asks what you
want when both are possible, states what it takes, then delegates. `--container` says "LCARS runs in a
...
`build` produces **two** images: the container you will run, and the toolchain twin its CI runner serves.
...
accounts and teams, mints the tokens, starts the container, registers a CI runner, and prints what it
...
| **`admiral`** | `toto1234` | **the system administrator.** It owns the container (sudo) and founded the forge. It runs the machine; it does not run the fleet — starting a fleet under it is refused by design. |
...
The same container, in a real terminal. From there:

```bash
fleet start        # start the fleet
fleet status       # what it is doing
lcars catalogue list  # which business catalogues this container carries
```

### Installing the demo catalogue

Straight out of the container you have one catalogue, `fleet`. A second, `web-demo`, is sitting on the forge as a
...
the container. It is an **admin** gesture: `lcars` can play it because this bench makes it a forge admin,
...
Removes the container, the forge, the runner and their volumes. Then, to reclaim the build space:
...
```bash
deploy/container -p lcars-nuit doctor   # what is provisioned, what drifted, and the gesture that fixes it
deploy/container -p lcars-nuit logs     # the container's own account of its boot
```

⚠ `-p lcars-nuit` is not optional here. `deploy/container` defaults to a project called `lcars`, and the
bench above creates one called `lcars-nuit` — without the flag you would be asking about a
deployment that does not exist. (`install.sh --container --bench -- --project <name>` changes it; the teardown line it
...
- **A container** — Debian, running the fleet's runtime (Elixir/OTP) and a web dashboard.
