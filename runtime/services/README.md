...
Fleet.Admiral.ToolchainReconciler       le BEAM, sous l'humain
  ...
  → runtime/bin/lcars-toolchain-converge  root  (binaire de PATH, pas un service)
```
...
`lcars-authority-ask`, que le BEAM et `runtime/bin/lcars` INVOQUENT. Rien ne les démarrait, ils
...
2026-08-27 sous `runtime/bin/`, avec leur nom définitif, comme `lcars` et `fleet_v2`. Le renommage
