# container — le boot du conteneur, côté produit
...
l'état dans le volume, l'entrypoint démarre le service. Ce dossier porte ce que le conteneur fait
...
- `init.sh secrets` : importe dans le répertoire privé du conteneur ce que le compose monte sous
  `/run/secrets` (`forge_master_token`, `forge_seed_password` — posés côté hôte par `container config`).
...
- `init.sh seat` : résout le siège (table `forge-uid.map`, sinon le #1 de la forge par le jeton
  ...
  de configuration », le conteneur reste debout pour que `container config` soit jouable.
