#!/usr/bin/env bash
# SOURCE: runtime/services/human.d/70-human.sh
# AUTHOR: DrDree
# STARDATE: 2026-07-05
...
TEMPLATE="$LCARS_PREFIX/etc/fleet.env.template"
...
apply() {
  ...
  if [[ ! -f "$ENV_FILE" ]]; then
    if [[ -r "$TEMPLATE" ]]; then
      ...
      mv -f "$tmp" "$ENV_FILE"
      LCARS_CHANGED=$((LCARS_CHANGED + 1))
      p_chg "fleet.env seedé depuis le template${FORGE_BASE_URL:+ (FORGE_BASE_URL=$FORGE_BASE_URL)} — désormais À L'HUMAIN, plus jamais réécrit ici"
    else
