...
# depot en est ecarte EN SILENCE — meme site-admin, meme proprietaire effectif, meme en `write`
# explicite : le PATCH rend 200, la liste reste vide, et le journal du conteneur ne dit rien.
# Mesure du 2026-09-18 sur le banc VIERGE 2004, quatre gestes d'une minute. La recette nomme donc
# une TEAM et AUCUN compte ; sa composition se derive du drapeau site-admin (`derive_admins`).
@test "les whitelists nomment une TEAM, et aucun compte" {
  local bloc
  bloc="$(awk '/^resource "gitea_repository_branch_protection" "tool_request" \{/,/^\}/' "$TF")"
  grep -q 'approval_whitelist_teams        = \[local.approvers_team\]' <<<"$bloc"
  grep -q 'push_whitelist_teams = \[local.approvers_team\]' <<<"$bloc"
  # les listes de COMPTES sont explicitement vides : les laisser absentes ferait croire a un oubli
  grep -q 'approval_whitelist_users        = \[\]' <<<"$bloc"
  grep -q 'push_whitelist_users = \[\]' <<<"$bloc"

  # le collaborateur de depot a disparu : il ne servait qu'a cette whitelist, et il n'y servait pas
  refute grep -q 'gitea_repository_collaborator' "$TF"
}

# Ce que le `write` de cette team ouvrirait par ailleurs — un push libre sur ce qui n'est pas
# protege — est referme ici. Sans ces deux protections, le droit donne pour SIGNER devient un droit
# d'ecrire partout.
@test "main et incidents sont protegees : le write des approbateurs n'est pas un push libre" {
  local bloc
  ...
  # Lui est NOMME et pas mis en team : il EST membre de `system`, donc Gitea le retient.
  bloc="$(awk '/^resource "gitea_repository_branch_protection" "incidents" \{/,/^\}/' "$TF")"
  grep -q 'push_whitelist_users       = \[var.system_account\]' <<<"$bloc"
  refute grep -q 'push_whitelist_teams' <<<"$bloc"
}
...
@test "la protection de tool_request : une approbation de la TEAM des approbateurs, reapprobation a chaque push, l admin ne contourne pas" {
  grep -q '^  required_approvals      = 1$' "$TF"
  grep -q '^  dismiss_stale_approvals = true$' "$TF"
  grep -q '^  block_merge_on_rejected_reviews = true$' "$TF"
  # seule cette team pousse directement (les semences) ; le compte systeme n'y est pas
  grep -q '^  enable_push          = true$' "$TF"
  grep -q '^  block_admin_merge_override = true$' "$TF"

  # LA TEAM DOIT EXISTER, sinon la whitelist nomme le vide et la protection est ouverte. Sa
  # COMPOSITION n'est pas l'affaire de cette recette : le geste la derive du drapeau site-admin.
  grep -q 'condition     = !local.system_play || contains(keys(local.teams), local.approvers_team)' "$TF"
}
