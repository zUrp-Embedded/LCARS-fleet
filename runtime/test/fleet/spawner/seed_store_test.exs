defmodule Fleet.Spawner.SeedStoreTest do
  ...
  test "checkpoint: workflow_map = passed builder, NOT the live jsonl's uuid (/clear rotation)",
    ...
    # /clear rotates the live UUID; the descriptor must still carry the supplied builder.
  ...
  test "slugify: RE-CONFRONTE au binaire vendor 2.1.220 (temoins mesures, pas rejoues)" do
    # Witness directories written by vendor 2.1.220 on 2026-08-03, not generated by our slugifier.
    # Refresh with `mix lcars.slug_witness --root <pods tree>` using normal subscription pods;
    # --print/stream-json is excluded by the launch policy (ADR-G), including its billing constraint.
    # A slug mismatch silently loses resume context. The second case detects separator collapsing.
  ...
  describe "recall keying — the seed of a ticket-scoped role is per TICKET" do
    test "recalling a ticket-keyed role WITHOUT a ticket is refused by name" do
      # Instance scope needs a ticket to select a unique session.
  ...
  test "restore: uuid escaping the pod_dir → REFUSED (fail-loud raise), no write outside the pod",
    ...
    # Four ../ segments cross the pod root from .claude/projects/<slug>/.
    # This checks the root boundary; restore does not require UUID shape or reject every ../.
  ...
  # The pod owns its writable tree; symlinks could make the daemon capture host files into seeds
  # or write outside the sandbox. Lexical path checks alone cannot detect those links.
  ...
  describe "JG-075 / JG-080 — illisible n'est pas vide, ni en lecture ni en ecriture" do
    test "JG-075 : repertoire de sessions ILLISIBLE → {:error, _}, pas :none", %{tmp: tmp} do
      ...
      # Root may still list the directory, bypassing the unreadable-directory case.
    ...
    # Use write-only 0o200: 0o000 also prevents writes and would hide the truncation regression.
    # Check surviving content, since returning an error alone does not prove preservation.
    test "JG-080 : un sidecar ILLISIBLE mais inscriptible n'est PAS tronque", %{
      ...
      # Root can bypass the read denial; the content check still applies after permissions restore.
