defmodule Fleet.SchemaCache do
  ...
  @moduledoc """
  Shared persistent cache for resolved schemas and boot-time artifacts.
  ...
  Raised failures are not cached; returned error tuples are cached as ordinary values.
  Retryable soft failures need success-only caching, as in `Fleet.CapProfile.Schema`.

  Cold callers may evaluate `fun` concurrently. A second read reuses an intervening value,
  but get/put is not atomic: multiple writers and differing returned values remain possible.
  No lock spans `fun`, avoiding lock-order deadlocks with nested cache calls. The function
  must tolerate repeated concurrent evaluation; file reads are allowed, one-time side effects
  are unsuitable. `{Fleet.SchemaCache, :miss}` is reserved as the internal miss sentinel.
  """
  ...
  def cached(persistent_key, fun) when is_function(fun, 0) do
    case :persistent_term.get(persistent_key, @miss) do
      @miss ->
        ...
        # Avoid overwriting a value observed after computation (and costly persistent-term
        # replacement). Another writer can still race after this read. put copies the value:
        # the writer returns its local term; cache readers share the stored term.
        case :persistent_term.get(persistent_key, @miss) do
          @miss ->
