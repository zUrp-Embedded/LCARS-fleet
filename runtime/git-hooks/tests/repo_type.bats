#!/usr/bin/env bats
# SOURCE: runtime/git-hooks/tests/repo_type.bats
# AUTHOR: drdree
# STARDATE: 2026-08-14
...
#
# ⚠ LE DERNIER CAS LIT LE VRAI `runtime/mix.exs` DU DEPOT. C'est lui le verrou : une fixture ne
...
# toplevel, son `runtime/mix.exs` suivi — et un hook git n'a de sens que la ou il y a un `.git`.
...
# Un depot git jetable dont on choisit le contenu de `runtime/mix.exs`.
make_repo() {
  ...
  if [[ -n "$mix_content" ]]; then
    mkdir -p "$dir/runtime"
    printf '%s\n' "$mix_content" > "$dir/runtime/mix.exs"
  fi
  printf '%s' "$dir"
}
...
@test "6-117: le CHEMIN seul ne suffit pas — un projet peut porter son propre runtime/mix.exs" {
  # Tester l'existence du fichier et pas son CONTENU rendrait LCARS de n'importe quel depot qui
  # s'organise avec un sous-projet `runtime/`. Le marqueur est l'identite de l'app, pas un dossier.
...
@test "6-117: l'ANCIEN sentinel ne ressuscite pas la branche — il ne veut plus rien dire" {
  ...
  touch "$repo/runtime/fleet-env.sh"
