...
**Date** : 2026-07-30
**Dernière révision** : 2026-08-12 (portage du VOCABULAIRE du protocole v1 — contrôle de session retiré : pas de handoff dans un pod)
...
|---|---|
| `quiet` | Réduit l'output à une ligne par action pour la session. Reset en fin de session. |
| `verbose` | Affiche le raisonnement intermédiaire pour chaque action. Reset en fin de session. |

---

## Règles du protocole — lire avant tout

**Ce fichier est une source de vérité primaire.** Il ne dérive d'aucun autre fichier. Fichier compagnon : `protocole-user.md` (personnalisation des mots-clés de session).
...
**Non personnalisable.** Aucun mot-clé défini ici ne peut être modifié, renommé, ou substitué. La seule personnalisation autorisée concerne deux mots-clés de contrôle de session dans `protocole-user.md`. Ce fichier est le contrat d'interaction, pas un template.
...
| 🔭 | `side quest:` | Mini-projet multi-session | Crée `plans/<slug>.md` dans ta face atelier (slug = titre court en kebab-case, dérivé du contenu) + avis court sur timing. Pas d'exécution immédiate. |
...
| Mot-clé | Contexte | Comportement |
|---|---|---|
| `note bien:` | Info à conserver | **Appelle `scratch`** avec la note. C'est un tool : il estampille, commite et pousse pour toi. Ne pas juste acquiescer — une info qu'on acquiesce sans l'écrire disparaît à la compaction, et tu ne sauras pas qu'elle a disparu. |
| `TODO:` | Idée ou action à réflexion | Dépose dans le `backlog.md` de ta face atelier. Pas d'exécution immédiate — c'est une idée, pas une commande |
| `TODO_now:` | Action immédiate | Traite immédiatement — la session peut se couper à tout moment |
| `backlog:` | Référence canonique | La file du projet : `backlog.md` de ta face atelier — référence only, aucune action déclenchée |
