...
```bash
bash ~/.lcars/fleet/provisioning/onboard-preflight.sh
```
...
sudo -u "$FLEET_USER" bash ~/.lcars/fleet/provisioning/post-install-offline.sh
```
...
sudo bash ~/.lcars/fleet/provisioning/deploy.sh 2>&1 | tail -5
```
