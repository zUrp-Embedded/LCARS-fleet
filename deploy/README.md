...
| 50-catalogues | any | any | le matériel des catalogues installés, convergé depuis la forge — un appelant mince du geste du produit `runtime/services/forge.d/catalogues.sh` : il passe ce que l'installeur sait (forge, répertoires), le geste rend le verdict |
| 60-deploy | wsl linux docker | any | la release du runtime : bâtie (ou reprise du kit) par `deploy/lib/deploy-release.sh` sous l'humain, verrouillée root:fleet 0750, câblée sur `/usr/local/bin` ; les intrus hors `release.manifest` sont retirés ; le canal (source ou kit) s'écrit après la pose |
| 61-forge-structure | wsl linux | wsl linux | la structure de la forge (orgs, comptes de rôle, teams, dépôt modèle) : roster dérivé de la release posée par 60, recette tofu copiée, initialisée hors-ligne et jouée par `forge-gestures.sh apply` ; le verdict lit ce que tofu dit avoir bougé |
| 62-runtime-helpers | wsl linux docker | any | les auxiliaires du runtime sur la machine : services, binaires du PATH, arbres embarqués à plat sous `/opt/lcars` (sans cache tofu, état, variables ni node_modules), client de terminal épinglé, réglage de shell (`/etc/skel/.bashrc` raccordé, PATH `~/.local/bin` dans `/etc/bash.bashrc`) |
| 63-forge-tokens | wsl linux | wsl linux | les jetons de rôle — un appelant mince du geste du produit `runtime/services/forge.d/tokens.sh` |
...
| 66-deck-oidc | wsl linux | wsl linux | le client OAuth2 du deck et `/etc/lcars/deck-oidc.json` — un appelant mince de `runtime/services/forge.d/deck-oidc.sh`, qui reçoit l'adresse annoncée, le port et les origines |
