...
@test "apply TAMPONNE la revision, a la racine que repo_root() de la copie retrouve" {
  ...
  stub_curl "peu importe"
  mod apply
  # ⚠ ON EPINGLE LA RELATION, PAS LE CHEMIN — ET ON LA DEMANDE A LA LIB AU LIEU DE LA RECOPIER.
  # Le tampon doit se poser LA OU `repo_root()` de la copie ira le chercher. Aujourd'hui ca tombe
  # sur `$LCARS_HELPERS_DIR`, mais par COINCIDENCE arithmetique : deplacer la copie d'un cran
  ...
  #
  # ⚠ CETTE LIGNE PORTAIT `readlink -f ../../..` — le decompte de crans RECOPIE de la lib. Elle a
  # rougi le jour ou l'installeur est sorti de `fleet/` et ou `repo_root()` est passe a deux crans :
  # un temoin qui se dit « immunise au chemin » l'etait au chemin, pas a l'arithmetique. Sourcer la
  # lib et l'INTERROGER supprime la seconde copie — c'est le meme geste que le « TEMOIN DU TEMOIN »
  # plus bas, qui derive deja `repo_root()` du decor plutot que de le supposer.
  local lu
...
racine_paquet() { # racine_paquet -> chemin d une racine de SOURCE qui se declare « paquet »
  ...
  # directory » — un decor qui ne se construit pas, donc quatre temoins rouges sur leur harnais et
  # non sur leur sujet.
