...
@test "le tiroir est NORMALISE — un chemin qui REMONTE est resolu, le tiroir annonce est le reel" {
  # ⚠ MESURE DU 2026-09-08, ET ELLE A COUTE SEPT MINUTES DE GATE. En packant depuis un worktree, le
  # tiroir naturel est `<worktree>/../lcars-packs`. Un outil de la chaine Debian (partie depuis)
  # echouait sur ce `..` apres le gate, la release et la doc — le plus tard possible pour la faute
  # la plus bete. La resolution normalise donc, et ce temoin le tient : le chemin que le pack
  # annonce est celui ou le `scp` d'apres ira chercher, quel que soit son cwd.
  local root="$BATS_TEST_TMPDIR/depot" out
  out="$(resolve_in "$root" "$BATS_TEST_TMPDIR/depot/../ailleurs")"
  [ "$(dirname "$out")" = "$BATS_TEST_TMPDIR/ailleurs" ] \
    || { echo "le tiroir garde son « .. » : $out"; return 1; }
  case "$out" in *..*) echo "un « .. » survit dans $out"; return 1 ;; esac
}

@test "un tiroir RELATIF devient absolu — le scp d'apres n'a pas le cwd du pack" {
  local out
