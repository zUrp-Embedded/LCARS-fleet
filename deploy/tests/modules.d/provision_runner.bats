...
# systeme, pose /opt/lcars — et n'a aucun desinstalleur.
...
@test "hors checkout git, la revision se lit dans le TAMPON — c'est ce qui rend une COPIE nommable" {
  ...
  # ⚠ ON DEMANDE LA REMONTEE A LA LIB, ON NE LA RECOPIE PAS — le commentaire ci-dessus promet de
  # « calculer le chemin comme la lib le fait », et cette ligne portait son decompte de crans en
  # double. Le jour ou l installeur est sorti de `fleet/`, la remontee est passee a DEUX crans et ce
  # temoin a rougi : la promesse etait dans la prose, pas dans le code.
  local root
...
#!/usr/bin/env bash
# SOURCE: deploy/modules.d/61-legue.sh
# AUTHOR: decor
# STARDATE: 2026-08-30
