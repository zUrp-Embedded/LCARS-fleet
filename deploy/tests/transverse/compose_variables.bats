...
setup() {
  ...
  COMPOSES=("$D/docker/docker-compose.yml" "$D/docker/docker-compose.bench.yml" "$D/docker/docker-compose.secrets.yml"
            "$D/docker/forge-compose.yml" "$D/docker/runner-compose.yml")
  POSEURS=("$D/container" "$D/docker/bench/bench-up.sh" "$D/docker/bench/bench-swap-image.sh" "$D/docker/bench/bench-down.sh"
           "$D/docker/forge-runner.sh" "$D/lib/forge-bootstrap.sh" "$D/lib/store.sh" "$D/installer-constants.env")
}
...
@test "chaque variable posée pour un compose du banc ou du runner est lue par un compose" {
  ...
  lues="$(sans_commentaires "${COMPOSES[@]}" | grep -oE '\$\{[A-Z][A-Z0-9_]+' | tr -d '${' | sort -u)"
  posees="$(
    for f in "$D/docker/bench/bench-up.sh" "$D/docker/bench/bench-swap-image.sh"; do
      sed -n '/env LCARS_IMAGE=/,/compose "\${COMPOSE_ARGS\[@\]}" create/p' "$f"
    done | grep -oE '(^|[[:space:]])[A-Z][A-Z0-9_]+=' | tr -d ' ='
    grep -oE "printf '[^']*" "$D/docker/forge-runner.sh" | grep -oE '(^|n|'"'"')[A-Z][A-Z0-9_]+=' | sed -E "s/^(n|')//; s/=$//"
  )"
