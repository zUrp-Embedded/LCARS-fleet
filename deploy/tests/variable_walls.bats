...
@test "MUR: le perimetre n'est pas VIDE — un balayage casse compte zero, comme un sans-faute" {
  ...
  # ⚠ LES CINQ ARBRES NE PARTENT PLUS DU MEME PREFIXE, ET CE CHEMIN EST ASSEMBLE A L'EXECUTION.
  # `"^$REPO/$t/"` ne contient la chaine d'aucun arbre : le recensement du demenagement, qui
  # cherchait des chemins litteraux, ne pouvait pas le voir. Depuis la separation, `deploy` est a la
  # racine et les quatre autres sont sous `fleet/` — un prefixe unique n'existe plus, et c'est
  # precisement le fait que ce mur doit refleter.
  local t
...
@test "MUR 5: le chemin du fichier de siege est le MEME partout, et le manifeste pose celui-la" {
  ...
  # ⚠ CETTE LISTE TRAVERSE LES DEUX LOGICIELS, ET C'EST TOUT SON INTERET : le chemin du fichier de
  # siege est declare des DEUX cotes de la frontiere. Depuis la separation, `$REPO` est la RACINE du
  # depot — les trois premiers sont sous `fleet/`, les trois derniers dans `deploy/`. Une liste ou
  # tout partait du meme prefixe n'existe plus, et c'est exactement ce que ce mur mesure.
  local sites=(
    "fleet/bin/fleet_v2"
