...
# ─── LE SUBSTRAT — la sonde sur un DECOR (`LCARS_SUBSTRATE_ROOT`) ──────────────────────────────
#
# Ces temoins verifient l'ORDRE et la DISCRIMINATION de `detect_substrate`, pas la presence reelle
# d'un fichier sur un terrain reel. (Le terrain `incus` et ses trois sondes ont vecu du 2026-09-08
# au 2026-09-11 ; sortis de la cible, ils sont partis avec leurs temoins.)

_sub_decor() { # un decor de substrat sous un chemin COURT
  SUBROOT="$(mktemp -d /tmp/lcars-sub-XXXXXX)"
  mkdir -p "$SUBROOT/proc"
}
...
@test "SUBSTRAT : un decor NU reste « linux », un noyau microsoft dit « wsl », .dockerenv gagne — la sonde n'invente pas un terrain" {
  _sub_decor
  ...
  _sub; [ "$output" = wsl ] || { echo "un noyau microsoft donne « $output »"; rm -rf "$SUBROOT"; return 1; }
  # docker D'ABORD : un conteneur dans un autre terrain se declare par ce qui l'entoure au plus pres
  touch "$SUBROOT/.dockerenv"
  _sub; [ "$output" = docker ] || { echo ".dockerenv sous un noyau microsoft donne « $output »"; rm -rf "$SUBROOT"; return 1; }
  rm -rf "$SUBROOT"
}
