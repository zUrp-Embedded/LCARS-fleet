...
routes_du_banc() { # routes_du_banc [is_admin] [code du Basic] [réponse du jeton] — une forge qui accepte l'humain « captain »
  local is_admin="${1:-true}" basic="${2:-200}" jeton='{"sha1":"op-abc"}'
  [[ -z "${3:-}" ]] || jeton="$3"
  forge_route PATCH /api/v1/admin/users/captain 200 '{"login":"captain"}'
  forge_route GET /api/v1/users/captain 200 "{\"login\":\"captain\",\"is_admin\":$is_admin}"
  forge_route GET /api/v1/user "$basic" '{"login":"captain"}'
  forge_route POST /api/v1/users/captain/tokens 201 "$jeton"
}
...
@test "forge_mount : compose up -d avec le projet, et port, bind, url racine dans l'environnement" {
  ...
  [ "$status" -eq 0 ]
  grep -qE "^DOCKER:compose --env-file [^ ]+/installer-constants\.env -f /x/forge-compose\.yml -p bob_9-forge up -d$" "$CALLS"
...
@test "forge_mount d'un banc : la surcouche marquée s'empile sur le compose, avec la base du banc" {
  ...
  [ "$status" -eq 0 ]
  grep -qE "^DOCKER:compose --env-file [^ ]+/installer-constants\.env -f /x/forge-compose\.yml -f /x/forge-compose\.bench\.yml -p bt-forge up -d$" "$CALLS"
...
@test "bench_human_seed : PATCH JSON au jeton master — mot de passe, login_name, source_id, et aucune adminité : la recette seule la pose" {
  forge_double_start
  routes_du_banc
  lib 'bench_human_seed "$FORGE_DOUBLE_URL" "$BATS_TEST_TMPDIR/master" captain toto32toto32'
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  local patch='select(.method == "PATCH" and .path == "/api/v1/admin/users/captain")'
  [ "$(forge_requests "$patch | .auth" | jq -r .)" = "token tok-master" ]
  [ "$(forge_requests "$patch | .ctype" | jq -r .)" = application/json ]
  [ "$(forge_requests "$patch | .body | fromjson")" = '{"login_name":"captain","source_id":0,"password":"toto32toto32","must_change_password":false}' ]
}
...
@test "bench_human_seed : l'adminité lue au jeton master, le mot de passe éprouvé en Basic, le jeton opérateur minté en Basic et son sha1 rendu" {
  ...
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ "$output" = "op-abc" ]
  [ "$(forge_requests 'select(.path == "/api/v1/users/captain") | .auth' | jq -r .)" = "token tok-master" ]
  [ "$(forge_requests 'select(.path == "/api/v1/user") | .auth' | jq -r .)" = "basic captain:toto32toto32" ]
  [ "$(forge_requests 'select(.path == "/api/v1/users/captain/tokens") | .auth' | jq -r .)" = "basic captain:toto32toto32" ]
  [[ "$(forge_requests 'select(.path == "/api/v1/users/captain/tokens") | .body | fromjson | .name' | jq -r .)" == bench-operateur-* ]]
  [ "$(forge_requests 'select(.path == "/api/v1/users/captain/tokens") | .body | fromjson | .scopes')" = '["write:repository","write:issue","read:organization","read:user"]' ]
}

@test "bench_human_seed : un mot de passe avec guillemet et barre oblique inverse arrive tel quel" {
  forge_double_start
  routes_du_banc
  lib 'bench_human_seed "$FORGE_DOUBLE_URL" "$BATS_TEST_TMPDIR/master" captain '"'"'mot"de\passe'"'"
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ "$(forge_requests 'select(.method == "PATCH") | .body | fromjson | .password' | jq -r .)" = 'mot"de\passe' ]
  [ "$(forge_requests 'select(.path == "/api/v1/user") | .auth' | jq -r .)" = 'basic captain:mot"de\passe' ]
}
...
@test "bench_human_seed : ni le jeton master ni le mot de passe dans l'argv ou l'environnement d'un enfant" {
  ...
  refute grep -qF "$(printf 'captain:toto32toto32' | base64 -w0)" "$DECOR_ENFANTS"
}

@test "bench_human_seed : un PATCH refusé est nommé avec son code HTTP, rien n'est rendu" {
  forge_double_start
  forge_route PATCH /api/v1/admin/users/captain 403 '{"message":"forbidden"}'
  run --separate-stderr bash -c "source '$PROV_LIB'; source '$LIB'; bench_human_seed \"\$FORGE_DOUBLE_URL\" \"\$BATS_TEST_TMPDIR/master\" captain pw"
  [ "$status" -eq 1 ]
  [ -z "$output" ]
  [ "$stderr" = "la forge refuse le compte « captain » (HTTP 403)" ]
}
...
@test "bench_human_seed : un humain que la structure n'a pas fait site-admin est nommé avec is_admin et le geste qui la rejoue, rien n'est rendu" {
  ...
  [ "$status" -eq 1 ]
  [ -z "$output" ]
  [[ "$stderr" == "« captain » n'est pas site-admin (is_admin=false) : son adminité est posée par la structure de la forge (gitea_user.human)"* ]]
...
@test "bench_human_seed : le geste qui rejoue la structure se tape tel qu'imprimé — docker exec avec l'humain dans le conteneur du banc, le délégué du poste sous --bench sinon" {
  ...
  run --separate-stderr bash -c "source '$PROV_LIB'; source '$LIB'; bench_human_seed \"\$FORGE_DOUBLE_URL\" \"\$BATS_TEST_TMPDIR/master\" captain pw \"\$BATS_TEST_TMPDIR/jeton\""
  [ "$status" -eq 1 ]
  [ "$(sed -n 's/^.* :  //p' <<<"$stderr")" = "$ws up --bench --humain-demo captain --only 61-forge-structure" ] || { echo "$stderr"; return 1; }
  # le conteneur du banc : chemin des gestes, root, et l'humain passé à docker exec
  run --separate-stderr bash -c "source '$PROV_LIB'; source '$LIB'; bench_human_seed \"\$FORGE_DOUBLE_URL\" \"\$BATS_TEST_TMPDIR/master\" captain pw '' bt-fleet-lcars-1"
  [ "$status" -eq 1 ]
  [ "$(sed -n 's/^.* :  //p' <<<"$stderr")" = "docker exec -u root -e LCARS_BUILTIN_HUMAN=captain bt-fleet-lcars-1 /opt/lcars/forge-gestures.sh apply" ] || { echo "$stderr"; return 1; }
  ...
  [ "$status" -eq 0 ]
  grep -qx 'GESTES:apply:LCARS_BUILTIN_HUMAN=captain' "$CALLS"
}
...
@test "bench_human_seed : un mot de passe que la forge refuse en Basic est nommé, rien n'est rendu" {
  ...
  [ "$status" -eq 1 ]
  [ -z "$output" ]
  [ "$stderr" = "« captain » ne s'authentifie pas avec le mot de passe posé" ]
}
...
@test "bench_human_seed : une réponse sans sha1 est nommée avec le corps de la forge, rien n'est rendu" {
  ...
  [ "$status" -eq 1 ]
  [ -z "$output" ]
  [ "$stderr" = "la forge n'a pas rendu de jeton opérateur pour « captain » : {\"message\":\"nope\"}" ]
}
...
@test "bench_human_seed : une forge muette au jeton opérateur est nommée sans reprendre la réponse précédente" {
  ...
  [ "$status" -eq 1 ]
  [ -z "$output" ]
  [ "$stderr" = "la forge n'a pas rendu de jeton opérateur pour « captain » :" ]
}
...
@test "bench_human_seed : un jeton posé qui s'authentifie encore est rendu tel quel, aucun jeton n'est minté" {
  ...
  lib 'bench_human_seed "$FORGE_DOUBLE_URL" "$BATS_TEST_TMPDIR/master" captain toto32toto32 "$BATS_TEST_TMPDIR/pose"'
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ "$output" = "op-pose" ]
  [ "$(forge_requests 'select(.path == "/api/v1/user") | .auth' | jq -r . | tail -n1)" = "token op-pose" ]
  [ -z "$(forge_requests 'select(.path == "/api/v1/users/captain/tokens")')" ]
}

@test "bench_human_seed : un jeton posé que la forge refuse est remplacé par un jeton minté" {
  forge_double_start
  forge_route PATCH /api/v1/admin/users/captain 200 '{"login":"captain"}'
  forge_route GET /api/v1/users/captain 200 '{"login":"captain","is_admin":true}'
  forge_route GET /api/v1/user 200 x1 '{"login":"captain"}'
  forge_route GET /api/v1/user 401 '{"message":"unauthorized"}'
  forge_route POST /api/v1/users/captain/tokens 201 '{"sha1":"op-neuf"}'
  printf 'op-revoque\n' > "$BATS_TEST_TMPDIR/pose"
  lib 'bench_human_seed "$FORGE_DOUBLE_URL" "$BATS_TEST_TMPDIR/master" captain toto32toto32 "$BATS_TEST_TMPDIR/pose"'
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ "$output" = "op-neuf" ]
...
@test "bench_human_seed : sans jeton posé à l'endroit nommé, un jeton est minté" {
  forge_double_start
  routes_du_banc
  lib 'bench_human_seed "$FORGE_DOUBLE_URL" "$BATS_TEST_TMPDIR/master" captain toto32toto32 "$BATS_TEST_TMPDIR/absent"'
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ "$output" = "op-abc" ]
}
