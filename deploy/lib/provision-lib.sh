...
# .source-revision`. Or `repo_root()` remonte DEUX crans depuis `<racine>/deploy/lib` (c'etait
# TROIS tant que l'installeur vivait sous `fleet/` ; la separation a change le decompte, pas la
# propriete qui compte ici — la remontee tombe sur la racine de l'arbre pose) — donc
