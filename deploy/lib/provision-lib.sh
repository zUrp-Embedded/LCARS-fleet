...
# ─── LE CANAL — QUI A POSE LE PRODUIT : `source` ou `kit` ────────────────────────────────────────
#
...
#
# Un seul poseur du produit sur ce rail, `60-deploy`, qui ecrit le canal apres la pose ; les autres
# modules convergent des comptes, des groupes, des unites, une forge — pas des fichiers du produit —
# et aucun module ne LIT le canal. Le troisieme canal, `deb` (un paquet dont dpkg possedait les
# arbres, et sous lequel 10/44/46/60/62 mesuraient sans poser), est parti le 2026-09-11 avec la
# chaine .deb.
#
# ⚠ ET UN CANAL SUR UN AUTRE EST UN REFUS, JAMAIS UNE CONVERSION. La porte et `workstation` lisent
# le fait `channel=` du preflight et refusent en nommant le geste. Le meme canal est une mise a
# jour : on continue.
...
# prov_channel -> stdout : `source` | `kit` | `aucun`, et POSE `PROV_CHANNEL` a la meme valeur.
# Une valeur hors des deux est un `p_fail` NOMME : rien sur stdout, `PROV_CHANNEL` vide, rc 1.
...
# du preflight. Meme piege que `advertise_addr`, meme raison.
prov_channel() {
  local v
  PROV_CHANNEL=""
  if [[ ! -e "$PROV_CHANNEL_FILE" ]]; then
    # ⚠ SANS TAMPON, IL Y A DEUX MACHINES : jamais posee (aucun), ou posee AVANT le tampon (un kit ou une
    # source d'avant le lot 2 — mesure 2003, 2026-09-05). Un produit present sans tampon est
    # « inconnu » : un kit ou une source le reprend et ecrit le tampon.
  ...
  p_fail "canal d'installation illisible : $PROV_CHANNEL_FILE porte « $v », attendu source ou kit — corrige-le à la main, ou retire-le si cette machine n'a jamais été posée"
  return 1
}

# prov_channel_write <source|kit> — atomique, au mode et au proprietaire que la TABLE declare
# (0644 root:root), par le SEUL poseur du produit sur ce rail (60-deploy).
prov_channel_write() {
  case "${1:-}" in
    source|kit) ;;
