...
#   deploy      L'INSTALLEUR LUI-MEME — il a change de liste en sortant de `fleet/`.
#
# ⚠ `deploy` VIENT DE PASSER DE LA PREMIERE LISTE A LA SECONDE, et ce n'est pas cosmetique : c'est
# la forme de la copie posee qui en depend. `repo_root()` remonte DEUX crans depuis le repertoire de
# la lib ; l'installeur pose ici doit donc se retrouver en `<PROV_ROOT>/deploy/lib/`, pas en
# `<PROV_ROOT>/fleet/deploy/lib/`. `EMBEDDED_ROOT` copie vers `$HELPERS_DIR/$n` — exactement cette
# forme — la ou `EMBEDDED` aurait pose sous `fleet/` et casse la remontee de tout apply rejoue
# depuis la copie, c'est-a-dire LE GESTE NOMINAL DU CONVERGEUR.
...
check() {
  ...
  # testait `$EMBEDDED_FLEET/deploy/provision`, c'est-a-dire `/opt/lcars/fleet/deploy/provision` —
  # un chemin que la separation installeur/runtime a rendu IMPOSSIBLE : `deploy` est passe dans
  ...
  #
  # ⚠ ET LA BOUCLE SUR `EMBEDDED` N'AVAIT PLUS D'OBJET : elle iterait sur les arbres de `fleet/`
  # pour juger un binaire de l'installeur, qui n'en fait plus partie. Le sujet est UN fichier, il se
  # nomme une fois.
