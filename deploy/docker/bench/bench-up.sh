...
# AUTHOR: DrDree
# STARDATE: 2026-08-03
# STATUS: geste de BANC — monte un banc COMPLET (forge jetable + conteneur + amorcage) en une commande
#
# ─── POURQUOI CE FICHIER EXISTE ─────────────────────────────────────────────────────────────────
# `bench-forge-bootstrap.sh` amene une forge NEUVE a l'etat "la fleet peut travailler dessus", et
# son en-tete dit pourquoi chacun de ses gestes existe. Mais il suppose la forge et le conteneur DEJA
# montees, connectees, et il faut le jouer DEUX FOIS (le semis des depots exige un token systeme
# que le conteneur ne minte qu'au boot SUIVANT le seed). Cette chorégraphie — quel projet compose, quel
# port, quel reseau brancher a quel moment, quand relancer le conteneur — vivait dans la memoire de la
...
#    saute le semis en le DISANT ; on relance le conteneur ; le second passage seme. Deux passages, pas
...
#    bind sont TOUS parametres et defaultent sur des valeurs libres. `--bind` couvre le conteneur ET la
#    forge : avant, il n'etait passe qu'au conteneur, la forge retombait sur
...
#         arguments/dependance · 2 la forge ne monte pas · 3 le conteneur ne monte pas · 4 amorcage
#         forge · 5 creds · 6 le verdict final ne passe pas — Y COMPRIS un runner DEMANDE qui ne
#         sert pas (image, token, enregistrement ou visibilite), ET un CONTENEUR qui a publie un echec
...
# d'amorcage quand le conteneur ne coute qu'un build, et `bench-swap-image.sh` existe pour exploiter
# cette asymetrie. Une fusion des projets rendait un `down -v` capable d'emporter la forge semee
# avec le conteneur — vu et corrige.
...
#   FORGE_URL        celle qu'on ANNONCE — le `ROOT_URL` de Gitea, le `FORGE_PUBLIC_URL` du
#                    conteneur, la ligne du recap. C'est celle qu'un ami compose depuis sa machine.
#
...
# LE CONTENEUR DOIT POUVOIR DIRE QUEL CODE IL PORTE, ET LE BANC DOIT LE LIRE AVANT DE L'ANNONCER.
#
# Ce n'est PAS un refus : le conteneur fonctionne, il est seulement muet sur son origine. On le dit
...
# ─── 2. le conteneur — create, brancher, PUIS demarrer (piege 1) ─────────────────────────────────────
...
store_ensure_volumes "$DOCKER_BIN" || die "magasin non pose — le conteneur ne peut pas se creer" 3
...
env LCARS_IMAGE="$IMAGE" \
  ...
  || die "le conteneur ne se cree pas (le reseau $FORGE_NET existe-t-il ? les volumes du magasin ?)" 3

# ⚠ LE SEMIS DU BINAIRE VENDOR A VECU ICI ET N'EXISTE PLUS. NE PAS LE REMETTRE.
"$DOCKER_BIN" compose "${COMPOSE_ARGS[@]}" start lcars || die "le conteneur ne demarre pas" 3
...
[[ "$("$DOCKER_BIN" inspect -f '{{.State.Health.Status}}' "$CONTAINER" 2>/dev/null)" == "healthy" ]] \
  || die "le conteneur ne devient pas healthy (docker logs $CONTAINER)" 3
...
say "relance du conteneur pour que le boot minte les role-tokens (forge.d/tokens, sur le token systeme)"
...
[[ "$("$DOCKER_BIN" inspect -f '{{.State.Health.Status}}' "$CONTAINER" 2>/dev/null)" == "healthy" ]] \
  || die "le conteneur ne redevient pas healthy apres relance" 3
...
if [[ "$WITH_CREDS" -eq 1 ]]; then
  ...
  "$DOCKER_BIN" exec "$CONTAINER" id -u "$HUMAN" >/dev/null 2>&1 \
    || die "le worker '$HUMAN' n'existe pas dans le conteneur apres la relance — le convergeur ne l'a pas
...
# et muet — le conteneur ne parlerait pas a la forge, et rien ne l'aurait dit. Le message nomme la cause,
...
[[ "$OP_TOKEN_OK" == "oui" ]] || die "token operateur absent chez $HUMAN apres DEUX passes — le conteneur ne
...
# ─── 6bis. LE VERDICT QUE LE CONTENEUR A PUBLIE SUR LUI-MEME ────────────────────────────────────────
# LE CONTENEUR MESURE SA PROPRE CONVERGENCE ET L'ECRIT, ET CE SCRIPT NE LA LISAIT PAS. L'entrypoint pose
# `/run/lcars-provision.rc` a chaque boot, precisement parce qu'un echec de convergence NE TUE PAS le
# conteneur : le conteneur doit rester joignable pour etre repare, donc il survit a son propre echec
# et se declare *healthy* (son healthcheck ne sonde que des ports : ssh + le deck). Sans cette lecture, un banc dont le
# conteneur ne peut demarrer AUCUN pod sortait `banc PRET` et rendait 0.
#
...
#   autre     au moins un echec — le conteneur tourne et ne produira RIEN : le banc n'est pas pret ;
...
case "$CONTAINER_PROV_RC" in
  ...
  "") CONTAINER_PROV_STATE="NON MESUREE — /run/lcars-provision.rc illisible dans le conteneur (il n'a peut-etre pas fini de converger)" ;;
  *)  CONTAINER_PROV_STATE="EN ECHEC (rc=$CONTAINER_PROV_RC) — le conteneur tourne et ne produira RIEN (\"$DOCKER_BIN exec $CONTAINER /opt/lcars/deploy/provision doctor\")"
      CONTAINER_PROV_OK=0 ;;
esac
...
# L'AUTORITE SE LIT DANS LE CONTENEUR, plus dans un fichier que ce banc aurait persiste. Elle y est
...
# Le conteneur passe AVANT le runner : un runner qui sert parfaitement un conteneur qui ne produit rien est
...
if [[ "$CONTAINER_PROV_OK" -ne 1 ]]; then
  VERDICT="banc PAS PRET — le CONTENEUR s'est declare en echec de convergence"
...
say "  op-token  : $OP_TOKEN_OK (~/.gitea_token de $HUMAN — la voie du conteneur vers la forge)"
say "  creds     : $CREDS_OK"
...
if [[ "$CONTAINER_PROV_OK" -ne 1 ]]; then
  die "le CONTENEUR a publie un echec de convergence ($CONTAINER_PROV_STATE) — banc INCOMPLET. Il tourne et
     reste joignable POUR ETRE REPARE, c'est l'arbitrage de l'entrypoint ; il ne produira rien
     tant que la convergence n'est pas verte. Aucun \`--no-…\` ne rend ce mode acceptable : un banc
     sans CI se choisit, un conteneur qui ne converge pas se subit" 6
fi
