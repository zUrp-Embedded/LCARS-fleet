...
# `compose down` efface les volumes, donc la forge, ses comptes, ses tokens et le /home du conteneur.
...
#
# UN BANC = TROIS PROJETS COMPOSE : le conteneur (`<projet>`), la forge (`<projet>forge`) et le
...
# ⚠ AVANT LE PREMIER APPEL A COMPOSE, PAS A LA FIN. Le compose du conteneur nomme ses volumes de
...
# La question juste n'est pas « le conteneur est-il la ? » mais « reste-t-il QUOI QUE CE SOIT de ce
# banc ? » — donc les trois conteneurs ET les volumes des trois projets. Les volumes comptent
# seuls : ce sont eux qui portent l'etat (la forge semee, le /home du conteneur), et `down -v` les
