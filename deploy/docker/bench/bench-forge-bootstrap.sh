#!/usr/bin/env bash
# SOURCE: deploy/docker/bench/bench-forge-bootstrap.sh
# AUTHOR: DrDree
# STARDATE: 2026-08-02
...
#         4 la structure (gestes du conteneur) · 5 le seed · 6 le verdict final ne passe pas
#         7 semis des repos
...
# Le conteneur porte desormais tofu, ses providers, la recette et les gestes (`/opt/lcars/
...
# le conteneur SERVIRA — l'arbre de l'hote peut avoir bouge depuis le build.
...
say "structure posee par le conteneur (org $ORG, teams, comptes, adhesions, propriete, depot modele)"
say "→ relance le conteneur (docker restart $CONTAINER) pour que 63-forge-tokens minte les role-tokens"
  ...
  if ! "$DOCKER_BIN" exec "$CONTAINER" id -u "$HUMAN" >/dev/null 2>&1; then
   ...
   de la forge, materialise par le convergeur a la relance). La passe 2 le posera."
    ...
    printf '%s\n' "$HUMAN_TOKEN" | "$DOCKER_BIN" exec -i -u "$HUMAN" "$CONTAINER" bash -c \
      ...
      || die "token operateur minte mais NON pose dans $CONTAINER — le conteneur ne pourra pas parler a la forge" 6
  fi
...
# La sonde vit DANS le conteneur avec le reste de la recette (l'hote n'en a plus de copie).
# SC2016 : les guillemets SIMPLES sont le geste. `$FORGE_BASE_URL` doit s'expanser DANS le conteneur,
...
# `fleet/lcars` = la source que le conteneur clone au boot (LCARS_SOURCE_REMOTE, la jambe runtime du
...
# Un projet neuf se peuple depuis le catalogue sur DISQUE, que le conteneur porte deja : il n'y a plus
...
if [[ "$SEED_REPOS" -eq 1 ]]; then
  ...
  if [[ -z "$SYS_TOKEN" ]]; then
    say "token systeme absent du conteneur — semis SAUTE (relance le conteneur puis rejoue ce script)"
  else
    ...
    # que le conteneur ne fait pas tourner.
